...
| Path | Purpose |
|---|---|
| `/opt/${PROJECT_NAME}/` | Production deployment of this project |
...
- **Vault symlink**: `${PROJECT_NAME}/vault` → `~/Documents/my_obsidian_vault`
...
3. Rebuild: `ssh server "cd /opt/${PROJECT_NAME} && docker compose up -d --build <service>"`.
...
### Recently Completed
- ✅ **Universal Project Harness**: Successfully ported strict TDD/Debugging constraints from `massage-bot`, implemented automated `/hydrate` and `/changelog` scripts, and authored the ${GIT_MAIN_BRANCH} `HARNESS_GUIDE.md`.
